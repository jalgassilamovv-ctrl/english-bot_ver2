...
# Озвучка — через gTTS (официальный публичный сервис Google Translate,
# без ключа и регистрации). Это самый надёжный бесплатный вариант: и
# edge-tts (голоса Microsoft), и StreamElements (голоса Amazon Polly)
# на практике оказались нестабильны при запросах с облачных серверов —
# Microsoft периодически меняет внутренние токены (ошибка 403), а
# StreamElements в какой-то момент начал требовать авторизацию
# (ошибка 401). gTTS звучит чуть менее "живо", чем нейросетевые голоса,
# зато не ломается непредсказуемо.
#
# TTS_TLD можно поменять для лёгкой вариации акцента: "com" (стандартный,
# по умолчанию), "co.uk" (британский), "com.au" (австралийский),
# "ca" (канадский).
TTS_TLD = os.environ.get("TTS_TLD", "com")
