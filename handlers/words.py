...
ВАЖНО про формат аудио: озвучка каждого слова отправляется как обычный
АУДИО-ФАЙЛ (send_audio, плеер-трек с названием), а НЕ как голосовое
сообщение (send_voice). У голосовых и кружочков-видео в Telegram есть
встроенное цепное автовоспроизведение — когда одно доигрывает, тут же
само включается следующее подряд идущее. У обычных аудио-файлов такого
поведения нет, поэтому каждое слово запускается только вручную, тапом
по своему плееру — то, что и нужно для рассылки из 15 слов подряд.
"""
import asyncio
import hashlib
import logging
...
async def send_one_word(bot, chat_id: int, w: dict):
    """Отправляет одно слово ОДНИМ сообщением: аудио-трек с произношением
    и подписью (эмодзи + слово + перевод + определение + пример) сразу
    под ним. Специально send_audio, а НЕ send_voice — иначе Telegram сам
    "цепляет" воспроизведение следующего голосового сразу после этого
    (см. комментарий в шапке файла). Если озвучить не получилось (сбой
    TTS) — отправляем тем же текстом обычным сообщением, чтобы слово
    всё равно дошло."""
