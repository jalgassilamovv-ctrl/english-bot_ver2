...
ВАЖНО про формат аудио: у Telegram есть ДВА встроенных механизма цепного
автовоспроизведения — один для голосовых/кружочков (send_voice), другой
для "музыкальных" аудио-файлов (send_audio, они собираются в общий плейлист
чата) — и оба продолжают само играть следующее сообщение, даже если между
ними есть текст. Поэтому озвучка отправляется как обычный ФАЙЛ-ДОКУМЕНТ
(send_document с .mp3), а не через send_voice/send_audio — у документов
нет своей "плейлист"-очереди в Telegram, поэтому воспроизведение
останавливается само, и следующее слово запускается только вручную.
"""
import asyncio
import hashlib
import logging
...
async def send_one_word(bot, chat_id: int, w: dict):
    ...
    (как файл-документ, .mp3) и подписью (эмодзи + слово + перевод +
    определение + пример) сразу под ним. Специально send_document, а НЕ
    send_voice/send_audio — у голосовых и "музыкальных" сообщений в
    Telegram есть своя цепочка автовоспроизведения (см. комментарий в
    шапке файла), а у обычных файлов-документов такой цепочки нет — оно
    останавливается само, и следующее слово нужно запускать вручную.
    Если озвучить не получилось (сбой TTS) — отправляем тем же текстом
    обычным сообщением, чтобы слово всё равно дошло."""
